...
let maybeContainedByTable (dishes: Dish seq) =
    let allergens =
        dishes
        |> Seq.map (fun d -> d.allergens)
        |> Set.unionMany

    Seq.map (fun allergen ->
        Seq.filter (fun d -> d.allergens.Contains(allergen)) dishes
...
let safeIngredients (dishes: Dish seq) =
    ...
    let suspipiousIngredients =
        (maybeContainedByTable dishes)
        |> Dict.values
        |> Set.unionMany
...
let countOccurences (dishes: Dish seq) (ingredients: string Set) =
    Seq.sumBy (fun d ->
        Set.intersect ingredients d.ingredients
        |> Seq.length) dishes
...
let solve2 (dishes: Dish seq) =
    ...
    while maybeInIngredient.Count > 0 do
        let (safeAssigned, unknown) =
            Map.partition (fun _ v -> Seq.length v = 1) maybeInIngredient
...
let solution1 =
    safeIngredients input |> countOccurences input
