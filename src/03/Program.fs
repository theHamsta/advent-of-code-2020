...
let getData file = readLines file |> Seq.map parseLine

let solution (data: seq<array<Field>>) =
    data
    |> Seq.mapi (fun row line -> line.[row * 3 % line.Length])
    |> Seq.filter (fun field -> field = Tree)
    |> Seq.length

let countTrees (data: seq<array<Field>>) moveFn rowPredicate =
    data
...
let solution2 data =
    ...
    moveOffsets
    |> List.map
        ((fun (dx, dy) -> countTrees data (moveFn dx dy) (rowPredicate dy))
         >> bigint)
    |> Seq.reduce (*)
